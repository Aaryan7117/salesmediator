/**
 * Rep My Leads — assigned leads list, same card format as admin leads.
 */
...
export default function RepMyLeads() {
  ...
  if (leads.length === 0) {
    return (
      <View style={styles.centered}>
        ...
        <Text style={styles.emptyText}>No leads assigned to you yet</Text>
        <Text style={styles.emptySubtext}>Your admin will assign leads from the pipeline</Text>
      </View>
    );
  }
        ...
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            <View style={styles.avatar}>
              <Text style={styles.avatarText}>
                {(item.persona || "?").split(" ").map(w => w[0]).join("").toUpperCase().slice(0, 2)}
              </Text>
            </View>
          ...
          <View style={styles.scoreBarBg}>
            <View style={[styles.scoreBarFill, {
              width: `${item.intent_score}%`,
              backgroundColor: STATE_COLORS[item.intent_state] || "#6B7280",
            }]} />
          </View>
